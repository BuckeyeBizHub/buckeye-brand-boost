    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-ohio-grey-dark/[0.97] backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.3)]" : "bg-transparent"}`}>
        ...
        <div className="hidden md:flex items-center gap-10">
          {links.map((l) => (
            <a
              key={l}
              href={`#${l.toLowerCase()}`}
              className="relative text-[0.8rem] font-bold text-primary-foreground/55 hover:text-primary-foreground tracking-[0.15em] uppercase transition-colors duration-300 after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-primary after:transition-all after:duration-300 hover:after:w-full"
            >
              {l}
            </a>
          ))}
      ...
      {open && (
        <div className="md:hidden bg-ohio-grey-dark/[0.98] backdrop-blur-xl border-t border-primary-foreground/10 pb-5 animate-fade-in">
          {links.map((l) => (
            <a key={l} href={`#${l.toLowerCase()}`} onClick={() => setOpen(false)} className="block px-6 py-3.5 text-sm font-bold text-primary-foreground/55 hover:text-primary active:text-primary transition-colors uppercase tracking-widest">
              {l}
            </a>
          ))}
